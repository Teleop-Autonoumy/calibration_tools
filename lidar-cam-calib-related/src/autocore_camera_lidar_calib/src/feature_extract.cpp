...
void lidar_feature_extract(void)
{
    ...
    if (inliers_chess->indices.size () == 0)
    {
        PCL_ERROR ("Could not estimate a planar model for the given dataset.");
        return ;
    }
...
void camera_feature_extracrt(void)
{
    ...
    cv::Size2d patternSize(0.108,0.108);
    ...
    float scale = std::sqrt((float)(width * height) / (640.0 * 480.0));
    cv::Mat scrib;

    if (scale > 1.0) {
        cv::resize(gray_, scrib, cv::Size((int)(width / scale), (int)(height / scale)));
    } else {
        scrib = gray_;
    }

    float x_scale = (float)width / (float)scrib.cols;
    float y_scale = (float)height / (float)scrib.rows;
    ...
    bool patternfound = cv::findChessboardCorners(scrib, patternNum, corners,
                                                          CV_CALIB_CB_ADAPTIVE_THRESH + CV_CALIB_CB_FILTER_QUADS + CV_CALIB_CB_NORMALIZE_IMAGE + CV_CALIB_CB_FAST_CHECK);
    ...
    std::cout << "Check corner number " << corners.size() << std::endl;
    ...
    else
    {
        ...
        int scale = 1;
        grid3dpoint.push_back(cv::Point3f(0.108*scale,  0.108*scale,  0.0));
        grid3dpoint.push_back(cv::Point3f(0.108*scale,  0.0*scale,  0.0));
        grid3dpoint.push_back(cv::Point3f(0.108*scale,  -0.108*scale,  0.0));
        grid3dpoint.push_back(cv::Point3f(0.0*scale,  0.108*scale,  0.0));
        ...
        //     for (int i =8; i>=1;i--)
        //     {
        //         grid3dpoint.push_back(cv::Point3f(j*0.108, i*0.108, 0.0));
        //     }
        // }
        ...
        scale = 1;
        boardcorners.push_back(cv::Point3f(0.216*scale,  0.216*scale,  0.0));
        boardcorners.push_back(cv::Point3f(0.216*scale, -0.216*scale,  0.0));
        boardcorners.push_back(cv::Point3f(-0.216*scale, 0.216*scale, 0.0));
        boardcorners.push_back(cv::Point3f(-0.216*scale, -0.216*scale, 0.0));
        
        // boardcorners.push_back(cv::Point3f(0.108*7,  0.108*9,  0.0));
        // boardcorners.push_back(cv::Point3f(0.108*7, 0.0,  0.0));
        // boardcorners.push_back(cv::Point3f(0.0, 0.108*9, 0.0));
        // boardcorners.push_back(cv::Point3f(0.0, 0.0, 0.0));

        cv::projectPoints(boardcorners, rvec_c, tvec_c, camera_instrinsics_, distortion_coefficients_, image_bc_points);
        cv::projectPoints(grid3dpoint, rvec_c, tvec_c, camera_instrinsics_, distortion_coefficients_, reproj);
        cout<<"rvec_c"<<rvec_c<<endl;
        cout<<"tvec_c"<<tvec_c<<endl;
        
        for (int i = 0; i < image_bc_points.size(); i++)
        {
            cv::circle(image_, image_bc_points[i],10, CV_RGB(255, 0, 0), -1);
            // cout << i << " " << image_bc_points[i].x << " " << image_bc_points[i].y << endl;

        }

        for (int i = 0; i < reproj.size(); i++)
        {
            cv::circle(image_, reproj[i], 5, CV_RGB(0, 255, 0), -1);
            // cout << i << " " << reproj[i].x << " " << corners[i].y << endl;
        }
